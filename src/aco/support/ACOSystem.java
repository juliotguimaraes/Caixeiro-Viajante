...
public class ACOSystem {
	...
	public ACOSystem(ArrayList<Double> inputList) {
	...
	private void initializeMatrixes(ArrayList<Double> inputList) {
		...
		for (int i = 0; i < totalVertices; i++) {
			for (int j = 0; j < totalVertices; j++) {
				adjacencyMatrix.get(i)
						.add(inputList.get(i * totalVertices + j));
	...
	public void run() throws InterruptedException {
		...
		for (int i = 0; i < numIterations; i++) {
			...
			for (int j = 0; j < numAnts; j++) {
				Ant ant = new Ant(adjacencyMatrix, pheromoneMatrix, alpha,
						beta, antPheromoneDepositAmount, neighbourCount,
						exploitationFactor);
		...
		// globalBestSolution =
		// randomLocalSearch(globalBestSolution.getReversedCopy());
		// }
	...
	private void globalPheromoneUpdate(Solution localBestSolution) {
		...
		for (int i = 0; i < localBestSolution.getSize() - 1; i++) {
			...
			if (value1 < maxPheromoneThreshold) {
				pheromoneMatrix.get(localBestSolution.getNodeByIndex(i)).set(
						localBestSolution.getNodeByIndex(i + 1), value1);
			}
			if (value2 < maxPheromoneThreshold) {
				pheromoneMatrix.get(localBestSolution.getNodeByIndex(i + 1))
						.set(localBestSolution.getNodeByIndex(i), value2);
			}
		}
	}

	// this is the first local search procedure
	private Solution randomLocalSearch(Solution localBestSolution) {
		...
		// int randomPointIndex = random.nextInt(localBestSolution.getSize() -
		// 2) + 1;
		// int randomPointIndex = random.nextInt(localBestSolution.getSize() /
		// 2) + localBestSolution.getSize() / 2 - 1;
		...
		ArrayList<Integer> threeNeighbourVertices = localBestSolution
				.getThreeNeighbourVerticesFromHalfToEnd();
		if (threeNeighbourVertices.isEmpty()) {
			return localBestSolution;
		}

		Integer targetNode = threeNeighbourVertices.get(random
				.nextInt(threeNeighbourVertices.size()));
		...
		// int partialCopySize = (localBestSolution.getSize() / 2) +
		// random.nextInt(localBestSolution.getSize() / 2);
		int partialCopySize = localBestSolution.getNodeList().indexOf(
				targetNode) + 1;
		// System.out.println(partialCopySize);

		Solution partialSolution = localBestSolution
				.getPartialCopy(partialCopySize);
		// System.out.println(partialSolution);
		Integer tabuNode = localBestSolution.getNodeByIndex(partialCopySize);

		Ant wiseAnt = new Ant(adjacencyMatrix, pheromoneMatrix, alpha, beta,
				antPheromoneDepositAmount, neighbourCount, exploitationFactor);
		...
		if (wiseAnt.getSolution().getCost() > localBestSolution.getCost()) {
			// System.out.println("LOCAL SEARCH SUCCESS: " +
			// localBestSolution.getCost() + "->" +
			// wiseAnt.getSolution().getCost());
			// System.out.println("PREVIOUS BEST " + localBestSolution);
			// System.out.println("TABU NODE " + tabuNode + " AT INDEX " +
			// partialCopySize);
	...
	public void setDaemonPheromoneDepositAmount(
	...
	// the second local search procedure
		...
		for (int i = 0; i < totalVertices; i++) {
			if (!improvedSolution.hasVertex(i)) {
				...
				while (true) {
					...
					if (adjacencyMatrix.get(currentVertex).get(i) > 0
							&& adjacencyMatrix.get(i).get(nextVertex) > 0) {
						double prevCost = adjacencyMatrix.get(currentVertex)
								.get(nextVertex);
						double newCost = adjacencyMatrix.get(currentVertex)
								.get(i)
								+ adjacencyMatrix.get(i).get(nextVertex);
						if (newCost >= prevCost) {
							improvedSolution
									.insertAfterVertex(currentVertex, i);
							break;
						}
					}
					j++;
